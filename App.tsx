...
function DuanOS() {
  ...
  const renderEntry = (e: Entry) => <View key={e.id} style={s.entry}>
    ...
    </View>{e.kind === 'ledger' && <Text style={[s.money, e.direction === 'income' && { color: C.green }]}>{e.direction === 'transfer' ? '↔ ¥' : e.direction === 'income' ? '+' : '−'}{cash(e.amountCents!)}</Text>}
  </View>;
    ...
    <Modal visible={open} animationType="slide" presentationStyle="pageSheet" onRequestClose={close}>
      ...
      <ScrollView contentContainerStyle={s.content} keyboardShouldPersistTaps="handled">
        {!draft ? <><Text style={s.subtitle}>一次记录一件事。支持记账、待办、定时提醒和每日复盘。</Text><Field label="输入内容" value={raw} onChange={setRaw} multiline placeholder="例如：微信支付午饭25元" /><Button title="识别内容" disabled={!raw.trim()} onPress={() => { setDraft(recognize(raw)); setError(''); }} /></> : <>
          ...
          {draft.kind === 'ledger' && <><View style={s.row}>{(['expense', 'income', 'transfer'] as const).map(direction => <Pressable accessibilityRole="button" key={direction} onPress={() => patch({ direction })} style={[s.chip, draft.direction === direction && s.chipActive]}><Text style={{ color: draft.direction === direction ? '#FFF' : C.muted }}>{direction === 'expense' ? '支出' : direction === 'income' ? '收入' : '转账'}</Text></Pressable>)}</View><Field label="金额（元）" value={draft.amount} onChange={amount => patch({ amount })} numeric />{draft.direction === 'transfer' ? <><Field label="转出账户" value={draft.account} onChange={account => patch({ account })} placeholder="例如：工资卡" /><Field label="转入账户" value={draft.counterpartyAccount} onChange={counterpartyAccount => patch({ counterpartyAccount })} placeholder="例如：微信零钱" /></> : <Field label="账户" value={draft.account} onChange={account => patch({ account })} placeholder="例如：工资卡、微信零钱" />}</>}
