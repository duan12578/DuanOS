...
// Cloudflare Workers Production currently caps PBKDF2 iterations at 100,000.
...
export async function verifyOwnerPassword(password: string, encoded: string): Promise<boolean> {
  const match = HASH_PATTERN.exec(encoded.trim());
  if (!match || !password || Number(match[1]) !== PBKDF2_ITERATIONS) return false;
  try {
    ...
    const bits = await crypto.subtle.deriveBits({ name: 'PBKDF2', hash: 'SHA-256', salt: salt as BufferSource, iterations: PBKDF2_ITERATIONS }, material, 256);
